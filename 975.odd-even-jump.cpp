 ...
 * j such that arr[i] <= arr[j] and arr[j] is the smallest possible value. If
...
public:
    int oddEvenJumps(vector<int>& arr) {
        ...
        for (int i = n-2; i >= 0; --i) {
            auto oddIt = valToIndex.lower_bound(arr[i]);
            if (oddIt != valToIndex.end()) {
                oddJump[i] = evenJump[oddIt->second];
            }
            
            auto evenIt = valToIndex.upper_bound(arr[i]);
            if (evenIt != valToIndex.begin()) {
                --evenIt;
                evenJump[i] = oddJump[evenIt->second];
            }
